...
for (const mode of ['insert', 'keys'] as const) {
  test(`sentences started in lowercase (${mode}) still score as correct`, async ({ page }) => {
    const { rid, text } = await begin(page)
    const lowered = text.replace(/(^|[.!?] +)([A-Z])/g, (_, gap: string, letter: string) => gap + letter.toLowerCase())
    const sentences = text.split(/(?<=[.!?]) +/).length
    // Word-sized insertions like a swipe keyboard, or one letter at a time like a tapped one.
    await typeText(page, lowered, mode)
    await expect(page.getByRole('heading', { name: 'Question' })).toBeVisible()
    const m = (await roundById(page, rid)).round.metrics!
    expect(m.accuracy).toBe(100)
    expect(m.accepted_count).toBe(text.length)
